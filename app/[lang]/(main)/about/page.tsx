...
export default async function AboutPage({
  ...
  return (
    <div className="mx-auto flex w-full max-w-2xl flex-1 flex-col gap-6 px-4 py-16 text-sm leading-relaxed text-foreground/90">
      ...
      <section className="flex flex-col gap-2">
        <h2 className="text-lg font-semibold text-foreground">콘텐츠는 어떻게 관리하나요</h2>
        <p>
          모든 서비스는 등록 전 운영자가 해당 기관의 공식 홈페이지와 공개 자료를
          직접 확인해 작성하고, 초안 작성 단계에서는 AI를 참고 도구로 활용합니다.
          등록 이후에도 매주 일요일 밤 등록된 모든 서비스의 링크 상태를 자동으로
          점검하는 절차를 거칩니다 — 접속이 안 되거나 오류가 발생하는 링크가 발견되면 즉시
          관리자에게 보고되어 빠르게 확인·수정합니다. 각 서비스 페이지에는 정보를
          마지막으로 확인한 날짜(최종 확인일)를 표시하며, 사이트 하단에서 전체
          링크를 마지막으로 점검한 날짜도 확인할 수 있습니다. 특히 실제 상황에서
          어떤 순서로 서비스를 이용해야 하는지 정리한{' '}
          <Link href={localeHref(lang, '/guides')} className="underline underline-offset-2">
            가이드
          </Link>
          는 등록된 서비스 여러 개를 실제 상황에서 쓰는 순서대로 엮어 운영자가
          직접 구성한 것으로, 개별 서비스 소개와 달리 &ldquo;무엇부터 해야
          하는지&rdquo;에 초점을 둡니다.
        </p>
      </section>
