...
async function resolvePost(params: PageProps['params']) {
  ...
  return getBoardPostById(postId, 'discover', board);
}
...
export default async function DiscoverBoardPostDetailPage({ params }: PageProps) {
  const post = await resolvePost(params);
  if (!post) notFound();

  return (
    <div className="mx-auto flex w-full max-w-2xl flex-1 flex-col px-4 py-16">
      <BoardPostDetail site="discover" basePath="/discover/board" post={post} />
    </div>
  );
}
